...
\usepackage[colorlinks=true,
            ...
            breaklinks=true,
            pdftitle={Tensor Structure in Cryptocurrency Microstructure: Low-Rank CP Decomposition from Binance OHLCV},
            pdfauthor={Murad Farzulla},
            pdfkeywords={Tensor decomposition, cryptocurrency markets, market microstructure, CP decomposition, Tucker decomposition, high-frequency data, factor models}]{hyperref}
...
\newcommand{\shorttitle}{Tensor Crypto Microstructure}
...
\fancyhead[R]{\small\href{https://doi.org/10.5281/zenodo.17626899}{DOI: 10.5281/zenodo.17626899}}  % Right: DOI
...
\begin{center}
{\Large\bfseries Tensor Structure in Cryptocurrency Microstructure: Low-Rank CP Decomposition from Binance OHLCV}\\[0.5em]
{\large\itshape Single-Venue Proof-of-Concept}\\[1em]
...
\vspace{0.5em}
\noindent \textbf{JEL Classification:} C38 (Classification Methods; Cluster Analysis; Principal Components), G12 (Asset Pricing; Trading Volume; Bond Interest Rates), C58 (Financial Econometrics)

% Metadata box with DOI
\metadatabox{10.5281/zenodo.17626899}

\vspace{0.5em}
\noindent \textbf{Interactive Dashboard:} Explore visualizations, methodology, and results interactively at \url{https://farzulla.org/research/tensor-defi/}
...
In financial markets, adversarial dynamics manifest as the tension between different market participants (high-frequency traders vs. long-term investors, centralized exchanges vs. decentralized protocols) and between market innovation and regulatory oversight. This paper applies tensor decomposition to cryptocurrency microstructure to reveal how multi-dimensional market dynamics encode these competing forces. By preserving the full tensor structure (time $\times$ asset $\times$ feature) rather than flattening to matrices, we uncover low-rank factor structure that reflects the multiplicative interaction between venue mechanics, asset-specific functions, and temporal regimes—interactions that traditional matrix methods destroy.

The 96.56\% explained variance achieved with rank-4 decomposition demonstrates that cryptocurrency markets, despite appearing chaotic, exhibit strong low-dimensional structure induced by systematic microstructure forces. This finding establishes tensor methods as essential for modeling markets where functional superposition (assets serving simultaneously as currency, collateral, gas tokens, and governance mechanisms) creates inherently multi-dimensional dynamics.

Future extensions will examine whether this low-rank tensor structure generalizes across different cryptocurrency market regimes (bull vs. bear markets, high vs. low volatility periods) and whether the extracted factors predict regime transitions. Ongoing work investigates multi-venue tensor decomposition to capture cross-exchange arbitrage dynamics and decentralized exchange (DEX) vs. centralized exchange (CEX) microstructure differences. The framework also suggests applications beyond finance: do political systems exhibit analogous low-rank structure when modeling voter preferences across multiple policy dimensions and time periods?
...
This proof-of-concept tests whether cryptocurrency markets exhibit low-rank tensor structure in a single-venue (Binance) setting. Tensor decomposition methods respect multi-way interactions by preserving structure that traditional matrix methods erase through flattening.
...
This single-venue proof-of-concept demonstrates tensor decomposition on multi-asset Binance OHLCV data, establishing feasibility for future multi-venue extensions. Our approach differs from prior work in three ways:
...
Building on the theoretical foundations of \citet{Babii2023Tensor} and the empirical success of \citet{Lettau2024ThreeD}, we establish that Binance OHLCV data exhibits low-dimensional tensor structure, and that tensor methods provide superior reconstruction accuracy while revealing economically meaningful factors invisible to traditional approaches.
...
\begin{figure*}[htbp]
    \centering
    \makebox[\textwidth]{\includegraphics[width=1.15\textwidth]{reconstruction_quality.pdf}}
    \caption{Reconstruction quality comparison across methods. Tensor methods (CP and Tucker) substantially outperform PCA, with reconstruction error declining more rapidly and plateauing at lower values. The negligible gap between CP and Tucker validates the true rank-4 structure assumption.}
    \label{fig:reconstruction-quality}
\end{figure*}

\begin{figure*}[htbp]
    \centering
    \makebox[\textwidth]{\includegraphics[width=1.15\textwidth]{rank_selection.png}}
    \caption{Explained variance vs. rank for different decomposition methods. Variance saturates rapidly beyond rank 4, with diminishing returns for additional components.}
    \label{fig:rank-selection}
\end{figure*}

Figures~\ref{fig:reconstruction-quality} and \ref{fig:rank-selection} demonstrate that tensor methods achieve superior reconstruction quality and that explained variance saturates rapidly beyond rank 4, with diminishing returns for additional components. This validates our rank selection approach and confirms that cryptocurrency market dynamics are dominated by a small number of latent factors.
...
\begin{figure*}[htbp]
    \centering
    \makebox[\textwidth]{\includegraphics[width=1.15\textwidth]{temporal_evolution.pdf}}
    \caption{Temporal evolution of the four extracted factors over the one-year period. Factor 3 (volatility regime) shows clear spikes during known market stress events (November 2024 election, March 2025 banking crisis), while Factor 4 exhibits regular 24-hour oscillations reflecting intraday microstructure. Factors 1 and 2 capture the alternation between Bitcoin dominance and altcoin season regimes.}
    \label{fig:temporal-evolution}
\end{figure*}

Figure~\ref{fig:temporal-evolution} visualizes the temporal dynamics of all four factors across the full year, revealing the clear regime structure that emerges from tensor decomposition. The alternating dominance of Factors 1 and 2 corresponds to observable market cycles, while Factor 3's event-driven spikes and Factor 4's periodic oscillations confirm the interpretability of the extracted latent structure.
...
On Binance data, tensor methods capture 4.6\% higher explained variance than PCA, suggesting multiplicative structure may exist in cryptocurrency microstructure. The improvement (from 92.31\% to 96.56\%) represents substantial economic value:
...
Future work will focus on several key extensions:

\begin{itemize}[leftmargin=*]
    \item \textbf{Multi-venue expansion}: Adding Coinbase, Kraken (CEX) and Uniswap, Curve (DEX) to test cross-exchange arbitrage dynamics and venue-specific microstructure effects. This will validate the full 4-way tensor hypothesis (time $\times$ venue $\times$ asset $\times$ feature) and enable analysis of CEX-DEX basis spreads.
    \item \textbf{Robustness validation}: Bootstrap confidence intervals for factor loadings, out-of-sample regime detection, expanded asset universe (10+ cryptocurrencies) to test rank scalability, and dynamic PCA baseline comparison across different market conditions.
    \item \textbf{Higher-dimensional features}: Incorporating blockchain-specific metrics such as MEV (Maximal Extractable Value), gas prices, block positions for Ethereum, and order book depth features to capture microstructure dynamics invisible in OHLCV data alone.
    \item \textbf{Information geometry framework}: Computing Riemannian metrics on the extracted factor manifold and using Ricci curvature for regime stress detection, enabling geometric characterization of market states beyond Euclidean distance measures.
    \item \textbf{Real-time deployment}: Implementing rolling window decomposition for live factor tracking and incremental tensor updates for streaming data, enabling practical applications in algorithmic trading and risk management.
\end{itemize}

These extensions will test whether the low-rank tensor structure observed in single-venue data generalizes to multi-venue settings and whether tensor methods provide robust out-of-sample regime predictions.
...
This proof-of-concept demonstrates that Binance cryptocurrency OHLCV data exhibits low-rank tensor structure (96.56\% variance with rank-4), outperforming PCA by 4.6pp. Tensor decomposition methods respect multi-way interactions between time, asset, and feature dimensions, while PCA and other matrix methods erase them through flattening.
...
All data, analysis code, figure generation scripts, and comprehensive documentation are publicly available at: \url{https://github.com/resurrexi/tensor-defi}. The repository includes complete implementation details, parameter configurations, and instructions for reproducing all results. Data sources include Binance historical OHLCV data (via CCXT library). Models are implemented in Python 3.13+ using TensorLy and can be executed on standard hardware. The code repository will be archived on Zenodo upon publication.
