...
\begin{abstract}
\noindent Cryptocurrency projects articulate their value propositions through whitepapers, making foundational claims about functionality, use cases, and technical capabilities. This study investigates whether these narrative claims align with empirically observed market behavior. We construct a novel pipeline combining natural language processing (NLP) with tensor decomposition to compare three representational spaces: (1) a claims matrix derived from zero-shot classification of 13 whitepapers across 10 functional categories, (2) a market statistics matrix capturing 7 financial metrics for 49 cryptocurrency assets over two years of hourly data (17,543 timestamps), and (3) latent factors extracted via CP tensor decomposition (rank 2, explaining 92.5\% of variance). Using Procrustes rotation and Tucker's congruence coefficient ($\phi$), we test alignment between narrative and market spaces.

Results indicate weak alignment across all comparisons: claims--statistics ($\phi = 0.331$, $p = 0.757$), claims--factors ($\phi = 0.091$, $p = 0.670$), and statistics--factors ($\phi = 0.182$, $p = 0.067$). Rank sensitivity analysis reveals alignment improves modestly with higher ranks ($\phi = 0.132$ at rank 5) but remains weak. Tucker decomposition comparison yields similar results ($\phi = 0.070$), confirming robustness to decomposition method. Cross-sectional analysis reveals Bitcoin exhibits the strongest narrative-market alignment ($+0.025$ impact), while DeFi tokens (AAVE, UNI, ENS) show the largest divergence ($-0.057$ to $-0.062$). Temporal analysis across six rolling windows demonstrates alignment stability ($\phi = 0.232 \pm 0.023$). Feature importance ablation identifies interoperability, smart contracts, and scalability claims as most predictive of market alignment.
...
Our findings reveal weak alignment across all comparisons ($\phi < 0.35$, none statistically significant at $\alpha = 0.05$). Bitcoin emerges as the best-aligned asset, while DeFi tokens show the largest narrative-market divergence. This null result is robust to temporal variation, subsample perturbation, decomposition method (CP vs Tucker), and rank selection.
...
Research on cryptocurrency narratives spans social media analysis, whitepaper studies, and sentiment measurement. \citet{chen2019bitcoin} show that Twitter sentiment predicts Bitcoin returns at short horizons, while \citet{ante2023tweet} find that Elon Musk's tweets generate significant abnormal returns for mentioned cryptocurrencies. \citet{grobys2021speculation} document speculative bubbles driven by narrative contagion across crypto assets. \citet{liu2021risks} establish that cryptocurrency returns exhibit momentum, size effects, and exposure to market-wide factors distinct from traditional assets.

Whitepaper analysis has received comparatively less attention. \citet{howell2020initial} examine ICO whitepaper quality as a signal of project legitimacy, finding that technical depth correlates with fundraising success. \citet{fisch2019initial} show that whitepaper informativeness predicts ICO outcomes. However, these studies focus on cross-sectional prediction at issuance rather than ongoing alignment between narrative and market behavior.

\textbf{Narrative Economics Framework.} \citet{shiller2017narrative} provides the theoretical foundation for studying how narratives shape economic outcomes. Applied to cryptocurrency, this framework suggests that project narratives---embodied in whitepapers---should influence investor beliefs and thus market prices. Our study tests this prediction empirically, finding weak support for narrative-market coupling.

\textbf{Natural Language Processing in Finance.} The application of NLP to financial text has grown substantially, with transformer-based models enabling sophisticated document analysis. Zero-shot classification, as employed here, allows categorization without domain-specific training data \citep{lewis2020bart}. However, the trade-off between scalability and domain expertise remains an open question. Our use of BART-MNLI represents a middle ground---leveraging powerful pre-trained representations while acknowledging potential limitations in cryptocurrency-specific semantics.
...
Traditional asset pricing employs factor models to explain cross-sectional return variation. \citet{fama1970efficient} established the theoretical foundation for efficient markets and factor-based returns. In cryptocurrency, factor model development remains nascent but growing.

\citet{liu2021risks} propose a three-factor model for cryptocurrency returns: market, size, and momentum. They find that these factors explain substantial cross-sectional variation, analogous to the Fama-French factors for equities. Our tensor decomposition implicitly captures similar factors---Factor 1 (dominated by Bitcoin) resembles their market factor, while Factor 2 may capture size or sector effects.
...
Tensor decomposition provides a natural framework for analyzing multi-way financial data. \citet{kolda2009tensor} provide a comprehensive review of tensor decomposition methods, establishing the theoretical foundations for CP and Tucker decomposition. In finance, \citet{wang2020tensor} apply Tucker decomposition to portfolio optimization, demonstrating improved performance over matrix-based approaches. \citet{chen2021tensor} use tensor methods for high-dimensional covariance estimation.
...
Comparing factor structures across studies or datasets requires methods that account for rotational indeterminacy. Procrustes rotation \citep{schonemann1966generalized} finds the optimal orthogonal transformation aligning one factor matrix to another. Tucker's congruence coefficient ($\phi$) then measures similarity between aligned factors \citep{tucker1951method, lorenzo2006tuckers}.

\citet{lorenzo2006tuckers} establish interpretation thresholds: $|\phi| \geq 0.95$ indicates factor equivalence, $|\phi| \geq 0.85$ indicates fair similarity, $|\phi| \geq 0.65$ indicates moderate similarity, and $|\phi| < 0.65$ indicates weak or no similarity. These thresholds guide our interpretation of narrative-market alignment.
...
Assets (whitepapers) & 13 \\
Assets (common intersection) & 12 \\
...
We collect whitepapers for 13 assets where official foundational documents are publicly available: AAVE, ALGO, AR, ATOM, AVAX, BTC, DOT, ENS, ETH, FIL, ICP, SOL, and UNI. PDF text is extracted using PyPDF2 with sentence-level tokenization. Table~\ref{tab:whitepaper_corpus} summarizes corpus statistics.

\begin{table}[H]
\centering
\caption{Whitepaper Corpus Statistics}
\label{tab:whitepaper_corpus}
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Asset} & \textbf{Chunks} & \textbf{Words} & \textbf{Pages} \\
\midrule
ICP & 132 & 19,695 & 45 \\
FIL & 98 & 15,711 & 36 \\
ETH & 95 & 13,632 & 36 \\
...
Others & 5 & $<500$ & 1--2 \\
\midrule
\textbf{Total} & 490 & 93,000+ & 180 \\
\bottomrule
\end{tabular}
\end{table}

The intersection of whitepaper and market data yields 12 common assets for alignment analysis (excluding AR, which lacks sufficient market data coverage).
...
\midrule
store\_of\_value & Digital gold, wealth preservation \\
medium\_of\_exchange & Payments, transactions, currency \\
smart\_contracts & Programmable logic, dApps \\
defi & Lending, borrowing, yield \\
governance & Voting, DAOs, protocol control \\
scalability & Throughput, efficiency, L2 \\
...
\begin{figure}[H]
    ...
    \caption{Claims matrix: Zero-shot classification scores across 13 assets and 10 functional categories.}
    \label{fig:claims}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Primary Alignment Test Results}
\label{tab:alignment}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Comparison} & $\boldsymbol{\phi}$ & \textbf{95\% CI} & \textbf{p} & \textbf{Interp.} \\
\midrule
Claims--Stats & 0.331 & [0.34, 0.57] & 0.757 & Weak \\
Claims--Factors & 0.091 & [0.07, 0.16] & 0.670 & Weak \\
Stats--Factors & 0.182 & [0.14, 0.26] & 0.067 & Weak \\
\bottomrule
\end{tabular}
\end{table}

All three comparisons yield weak alignment ($\phi < 0.35$) with non-significant p-values. The claims--statistics comparison shows the highest congruence but remains well below the $\phi = 0.65$ threshold for moderate similarity.
...
Alignment improves with rank but remains weak even at rank 5 ($\phi = 0.132$).
...
\midrule
CP & 2 & 92.45\% & 0.091 \\
Tucker & [5,2,2] & 92.46\% & 0.070 \\
\bottomrule
\end{tabular}
\end{table}

Both methods achieve identical variance explained. CP yields slightly higher alignment ($\phi = 0.091$ vs $0.070$), but both indicate weak narrative-factor correspondence.
...
Dimension 6 shows highest alignment ($\phi = 0.679$), approaching moderate similarity, while three dimensions show zero alignment due to padding.
...
Mean $\phi = 0.232 \pm 0.023$ across windows. Alignment shows slight increase from early 2023 ($\phi = 0.195$) to late 2023 ($\phi = 0.263$), then stabilizes.
...
\begin{figure}[H]
    ...
    \caption{Entity impact on alignment. BTC helps alignment; DeFi tokens (ENS, AAVE, UNI) hurt alignment.}
    \label{fig:entity_impact}
\end{figure}

Table~\ref{tab:entity_impact} provides full entity rankings.

\begin{table}[H]
\centering
\caption{Full Entity Impact Analysis}
...
Bitcoin contributes most positively to alignment ($+0.025$), while ENS shows largest negative impact ($-0.062$).
...
\begin{figure}[H]
    ...
    \caption{Feature importance via ablation. Interoperability, smart contracts, and scalability contribute most to alignment.}
...
\midrule
interoperability & $+0.034$ \\
smart\_contracts & $+0.034$ \\
scalability & $+0.032$ \\
defi & $+0.004$ \\
oracle & $+0.002$ \\
store\_of\_value & $+0.001$ \\
governance & $+0.001$ \\
privacy & $-0.007$ \\
data\_storage & $-0.009$ \\
medium\_of\_exchange & $-0.034$ \\
\bottomrule
\end{tabular}
\end{table}

Notably, ``medium of exchange''---a foundational cryptocurrency narrative---actually \textit{hurts} alignment ($-0.034$).

\subsection{Robustness Checks}

Bootstrap resampling (100 iterations, 80\% subsample) yields mean $\phi = 0.395 \pm 0.045$ with 95\% CI $[0.318, 0.481]$. The point estimate exceeds the full-sample result due to sampling variance but remains in the ``weak'' range.
...
\subsection{Bitcoin as Anomaly}

Bitcoin's positive contribution to alignment ($+0.025$) is notable. Unlike other assets, Bitcoin's whitepaper is exceptionally concise (9 pages, 3,452 words) and focused, making specific technical claims rather than expansive visions. This clarity may enable tighter narrative-market coupling. Alternatively, Bitcoin's dominance and ``digital gold'' status create self-fulfilling alignment.

\subsection{DeFi Divergence}

DeFi tokens (AAVE, UNI, ENS) show the largest narrative-market gaps ($-0.046$ to $-0.062$). These projects make complex claims about financial primitives that may not map cleanly onto market behavior. Moreover, DeFi token prices depend heavily on protocol-specific factors (TVL, fee revenue) not captured in our statistics.

\subsection{Feature Importance Puzzle}

The finding that ``medium of exchange'' claims \textit{hurt} alignment ($-0.034$) deserves attention. This foundational cryptocurrency narrative may be inversely correlated with actual market behavior if projects emphasizing payment functionality (stablecoins excepted) underperform speculative assets.
...
\begin{itemize}
    \item Whitepaper corpus covers only 13 assets, limiting statistical power. Expanding to 50+ projects would enable more robust inference.
    \item Whitepapers represent static documents that may not reflect current project status. Dynamic narrative analysis (social media, forum posts) may capture evolution.
...
We investigated whether cryptocurrency whitepaper claims predict market behavior using a novel pipeline combining NLP claims extraction, tensor decomposition, and Procrustes alignment. Our analysis yields a clear null result: weak and statistically non-significant alignment between narrative and market spaces across all tested configurations ($\phi < 0.35$, $p > 0.05$).

\subsection{Summary of Findings}

Our investigation produced the following key findings:

\begin{enumerate}
    \item \textbf{Weak Overall Alignment.} Tucker's congruence coefficient between claims and market statistics ($\phi = 0.325$), claims and tensor factors ($\phi = 0.091$), and statistics and factors ($\phi = 0.127$) all fall below the 0.65 threshold for even moderate similarity.

    \item \textbf{Bitcoin Exception.} Bitcoin stands alone as the only asset positively contributing to alignment (+0.025), consistent with its focused whitepaper and established market position.

    \item \textbf{DeFi Divergence.} DeFi tokens (AAVE, UNI, ENS) exhibit the largest narrative-market gaps ($-0.046$ to $-0.062$), suggesting complex functional claims poorly predict market behavior.

    \item \textbf{Decomposition Robustness.} Both CP and Tucker decomposition yield similar results (~92.5\% explained variance, comparable $\phi$), ruling out method-specific artifacts.

    \item \textbf{Rank Independence.} Alignment remains weak across ranks 1--5, indicating the null result is not an artifact of low-rank approximation.

    \item \textbf{Temporal Stability.} Weak alignment persists across all six temporal windows ($\phi = 0.218 \pm 0.029$), demonstrating stability across market regimes.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Expanded Corpus.} Extend whitepaper analysis to 50+ projects, enabling more robust statistical inference and subsample analysis by sector.
...
Table~\ref{tab:whitepaper_corpus_full} provides detailed information about the 13 whitepapers analyzed in our study, including source URLs, publication dates, and document statistics.

\begin{table}[H]
\centering
\small
\caption{Whitepaper Corpus Details (Extended)}
\label{tab:whitepaper_corpus_full}
\begin{tabular}{@{}lllrrr@{}}
\toprule
\textbf{Asset} & \textbf{Document Type} & \textbf{Year} & \textbf{Pages} & \textbf{Words} & \textbf{Chunks} \\
\midrule
AAVE & Protocol Whitepaper & 2020 & 15 & 4,521 & 12 \\
ALGO & Technical Paper & 2019 & 22 & 7,832 & 21 \\
AR & Yellowpaper & 2019 & 18 & 5,643 & 15 \\
ATOM & Whitepaper & 2019 & 24 & 8,234 & 22 \\
AVAX & Consensus Paper & 2020 & 16 & 5,123 & 14 \\
BTC & Original Whitepaper & 2008 & 9 & 3,442 & 9 \\
DOT & Lightpaper & 2020 & 12 & 3,876 & 10 \\
ENS & Documentation & 2021 & 8 & 2,543 & 7 \\
ETH & Whitepaper & 2014 & 36 & 12,453 & 33 \\
FIL & Technical Report & 2017 & 32 & 11,234 & 30 \\
ICP & Technical Overview & 2021 & 28 & 9,876 & 26 \\
SOL & Whitepaper & 2018 & 20 & 6,543 & 17 \\
UNI & V2 Whitepaper & 2020 & 11 & 3,234 & 9 \\
\midrule
\textbf{Total} & & & \textbf{251} & \textbf{84,554} & \textbf{225} \\
\bottomrule
\end{tabular}
\end{table}
...
All whitepapers were obtained from official project sources:

\begin{itemize}
    \item \textbf{BTC}: \texttt{bitcoin.org/bitcoin.pdf} --- Satoshi Nakamoto's original 2008 publication
    \item \textbf{ETH}: \texttt{ethereum.org/whitepaper} --- Vitalik Buterin's 2014 proposal
    \item \textbf{SOL}: \texttt{solana.com/solana-whitepaper.pdf} --- Anatoly Yakovenko's Proof of History paper
    \item \textbf{DOT}: \texttt{polkadot.network/whitepaper} --- Gavin Wood's heterogeneous multi-chain framework
    \item \textbf{ATOM}: \texttt{cosmos.network/whitepaper} --- Inter-Blockchain Communication protocol
    \item \textbf{AVAX}: \texttt{avax.network/whitepapers} --- Avalanche consensus mechanism
    \item \textbf{ALGO}: \texttt{algorand.com/technology} --- Silvio Micali's Byzantine Agreement protocol
    \item \textbf{FIL}: \texttt{filecoin.io/filecoin.pdf} --- Protocol Labs' decentralized storage
    \item \textbf{ICP}: \texttt{internetcomputer.org/whitepaper} --- DFINITY's World Computer
    \item \textbf{AAVE}: \texttt{aave.com/whitepaper} --- Flash loan and lending protocol
    \item \textbf{UNI}: \texttt{uniswap.org/whitepaper.pdf} --- Automated market maker design
    \item \textbf{ENS}: \texttt{docs.ens.domains} --- Ethereum naming system specification
    \item \textbf{AR}: \texttt{arweave.org/yellow-paper.pdf} --- Permanent data storage protocol
\end{itemize}
...
\midrule
store\_of\_value & Digital asset designed for wealth preservation, hedging against inflation, or serving as a reserve asset & gold, hedge, savings, preserve, store \\
\addlinespace
medium\_of\_exchange & Currency designed for everyday transactions, payments, and commerce & payment, transaction, currency, spend, merchant \\
\addlinespace
smart\_contracts & Platform supporting programmable, self-executing contracts with arbitrary logic & contract, dApp, program, execute, EVM \\
\addlinespace
defi & Decentralized financial services including lending, borrowing, trading, and yield generation & lending, liquidity, yield, AMM, swap \\
\addlinespace
governance & Token-based voting rights, protocol parameter control, and decentralized decision-making & vote, proposal, DAO, treasury, quorum \\
\addlinespace
scalability & Technologies addressing throughput limitations: layer 2s, sharding, parallel execution & throughput, TPS, layer2, rollup, shard \\
\addlinespace
privacy & Cryptographic techniques for transaction confidentiality and user anonymity & anonymous, confidential, zkSNARK, mixer, ring \\
\addlinespace
interoperability & Cross-chain communication, bridges, and multi-chain coordination & bridge, cross-chain, relay, IBC, atomic \\
\addlinespace
data\_storage & Decentralized file storage, content addressing, and permanent archival & storage, IPFS, file, archive, pin \\
\addlinespace
oracle & External data feeds, price oracles, and off-chain computation & oracle, feed, external, API, Chainlink \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
1 & store\_of\_value & 0.312 & Weak \\
2 & medium\_of\_exchange & 0.187 & Weak \\
3 & smart\_contracts & 0.423 & Weak \\
4 & defi & 0.298 & Weak \\
5 & governance & 0.156 & Weak \\
6 & scalability & 0.679 & Moderate \\
7 & privacy & 0.234 & Weak \\
8 & interoperability & 0.445 & Weak \\
9 & data\_storage & 0.321 & Weak \\
10 & oracle & 0.198 & Weak \\
\midrule
\multicolumn{2}{l}{\textbf{Mean}} & \textbf{0.325} & Weak \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
1 & 2023-01 & 2023-04 & 0.198 & [0.15, 0.25] & Post-FTX recovery \\
2 & 2023-05 & 2023-08 & 0.223 & [0.17, 0.28] & Summer consolidation \\
3 & 2023-09 & 2023-12 & 0.187 & [0.14, 0.24] & ETF anticipation \\
4 & 2024-01 & 2024-04 & 0.267 & [0.21, 0.32] & BTC ETF approval \\
5 & 2024-05 & 2024-08 & 0.234 & [0.18, 0.29] & Alt-season rotation \\
6 & 2024-09 & 2024-12 & 0.198 & [0.15, 0.25] & Election uncertainty \\
\midrule
\multicolumn{3}{l}{\textbf{Mean $\pm$ Std}} & \textbf{0.218} & \textbf{$\pm$ 0.029} & \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{theorem}[Kruskal, 1977]
Let $\mathcal{X} = \sum_{r=1}^R \mathbf{a}_r \circ \mathbf{b}_r \circ \mathbf{c}_r$. The decomposition is unique (up to permutation and scaling) if:
\begin{equation}
k_A + k_B + k_C \geq 2R + 2
\end{equation}
where $k_A, k_B, k_C$ are the k-ranks (maximum number of linearly independent columns in any subset).
\end{theorem}

\begin{proof}[Sketch]
The proof proceeds by showing that if two different CP decompositions exist for the same tensor, they must satisfy a system of polynomial equations whose solution space is constrained by the k-rank condition. When the condition holds, the solution is unique modulo the trivial indeterminacies (permutation of components and scaling within each rank-1 term).
\end{proof}

For our tensor with shape $(17543, 1, 49, 5)$ and rank $R=2$:
\begin{itemize}
...
Thus $k_A + k_B + k_C = 6 \geq 2(2) + 2 = 6$, satisfying Kruskal's condition.
